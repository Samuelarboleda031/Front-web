...
interface LoginPageSimplifiedProps {
  onRequestRegister?: () => void;
  onBackToLanding?: () => void;
}

export function LoginPageSimplified({ onRequestRegister, onBackToLanding }: LoginPageSimplifiedProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const rolId = formData.role === 'admin' ? 1 : 3;
      const result = await login(formData.email, formData.password, rolId);
      if (!result.success) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const rolId = formData.role === 'admin' ? 1 : 3;
      const result = await loginWithGoogle(rolId);
      if (!result.success) {
